...
class TextureMeshRenderer : public Component {
public:
    TextureMesh* pMeshData = nullptr;
    Material* pMaterial = nullptr;
    ID3D11Buffer* cBuffer = nullptr;

    TextureMeshRenderer(TextureMesh* mesh, Material* mat)
        : pMeshData(mesh), pMaterial(mat)
    {
    }
    ...
    void Render() override {
        ...
        if (aspect >= 1.0f)
            aspectCorrection = XMMatrixScaling(1.0f / aspect, 1.0f, 1.0f);
        else
            aspectCorrection = XMMatrixScaling(1.0f, aspect, 1.0f);
